...
$('#selectProforma').change(function() {
    let id = $("#selectProforma").val();
    $.ajax({
        type: 'POST',
        url:'/Proforma/Update_Proforma/',
        ...
        success: function(data){
            ...
            console.log(data['materiales'].length);
            console.log(data['item'].length);
            num_rows = data['materiales'].length + data['item'].length;
            $('#selectSolicitante').val(data['proformas']['SOLICITANTE']);
            $('#selectLugar').val(data['proformas']['LUGAR']);
            $('#txtTiempo').val(data['proformas']['TIEMPO']);
            $('#txtMateriales').val(data['materiales'].join('\n'));
            $('#txtCant1').val(data['item'][0]['CANTIDAD']);
            $('#txtDescripcion').val(data['item'][0]['DESCRIPCION']);
            $('#txtPrecioUnitario1').val(data['item'][0]['PRECIO_UNITARIO']);
            $('#txtPrecioTotal1').val(data['item'][0]['PRECIO_TOTAL']);
            $('#num_rows').text("N° de filas: " + num_rows);
        }
...
function Generar_Proforma(){
    ...
    let materiales = $('#txtMateriales').val().split("\n");
    let json_materiales = JSON.stringify(materiales);
    ...
    $('.Cantidad').each(function(index) {
        ...
        let precio_total = $('.precioTotal').eq(index).val();
        let item = {
            cantidad: cantidad,
    ...
    $.ajax({
        ...
        headers: {'X-CSRFToken': csrftoken},
        dataType: 'json',
        data:{solicitante: solicitante,lugar: lugar, materiales: json_materiales, tiempo:tiempo, lista_items:json_lista_items},
...
function updateTotalForRow(row) {
    ...
    document.getElementById('txtPrecioTotal' + row).value = precio_total.toFixed(2);
    UpdateTotal();
...
function contarSaltosDeLinea() {
    const texto = document.getElementById("txtMateriales").value;
...
document.getElementById("txtMateriales").addEventListener("input", contarSaltosDeLinea);
